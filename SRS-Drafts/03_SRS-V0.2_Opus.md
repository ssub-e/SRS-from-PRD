...
| **PRD 원본** | PRD v1.0 (2026-04-16) |
| **선행 SRS** | SRS-001 Rev 1.1 (PRD v0.1 기반, 폐기) |
...
> **PRD v1.0 핵심 전환:** 바이오 콜드체인(IoT 관제) 시나리오는 제품의 Core 정체성을 위해 **MVP 전 구간에서 배제**되었다.
...
| ID | 항목 | 설명 | PRD 참조 |
|---|---|---|---|
| SCOPE-IN-01 | F1. XAI 원클릭 리포트 추출 | 기상·트렌드·매출 변수가 융합된 XAI 발주 권장 PDF 리포트 자동 생성. SHAP 기반 외부 변수 Top 3 자연어 해설 포함 | PRD §4-1 F1, §3 Story 1 |
| SCOPE-IN-02 | F2. 쇼핑몰 API 원클릭 연동 모듈 | 카페24·스마트스토어 등 쇼핑몰 플랫폼 OAuth 기반 1-click 연동. 화주사 개발 개입 원천 차단 | PRD §4-1 F2, §3 Story 2 |
| SCOPE-IN-03 | F3. 적정 인원 역산 대시보드 | 예측 출고량 기반 일용직 적정 인원 산출 및 카카오 알림톡 자동 발송 (매일 16:00) | PRD §4-1 F3, §3 Story 2 |

#### 1.2.2 Out-of-Scope

| ID | 항목 | 배제 사유 | PRD 참조 |
|---|---|---|---|
| SCOPE-OUT-01 | F4. 콜드체인 차량 배차 및 온도 이탈 노선 재설정기 | **제품 방향성 산란 원인 — 전면 삭제** (Won't) | PRD §4-1 Won't |
| SCOPE-OUT-02 | 예산 지출 연동 What-IF 시뮬레이터 | 추후 확장 과제 (Won't) | PRD §4-1 Won't |

#### 1.2.3 Constraints

| ID | 제약사항 | PRD 참조 |
|---|---|---|
| CON-01 | 기상청 단기예보 API Rate Limit — 배치 큐잉 필수 | PRD §5-1 |
| CON-02 | 카페24 API Rate Limit: 분당 제한 — 캐시 레이어 필수 | PRD §5-1 |
| CON-03 | MVP 기간 인프라 월 비용: 무료~최대 월 100만 원 이내 (클라우드 무료 티어 우선 활용) | SRS 자체 보완 |
| CON-04 | F3(인원 역산)은 F2(API 연동 모듈) 선행 완료 필수 | PRD §3 Story 2 |
| CON-05 | PDF 리포트 양식은 파일럿 고객의 기존 결재 양식 수집 후 확정 | PRD §3 Story 1 AC 1.2 |
| CON-06 | 코어 예측 알고리즘은 SaaS API 과금 모델로 IP 보호 — 소스코드 외부 노출 금지 | PRD §10 |
| CON-07 | 멀티테넌트 격리는 MVP 기간 논리적(스키마) 격리 수준 적용 | PRD §5-2 |

#### 1.2.4 Assumptions

| ID | 가정 | PRD 참조 |
|---|---|---|
| ASM-01 | 카페24·스마트스토어 API가 현재 스펙대로 최소 12개월 유지된다 | SRS 자체 보완 |
| ASM-02 | 기상청 단기예보 API 무료 이용이 MVP 기간 동안 지속된다 | SRS 자체 보완 |
| ASM-03 | 파일럿 고객(4사)이 파일럿 기간 전까지 확보 가능하다 | PRD §8-1 |
| ASM-04 | 클라우드 무료 티어(AWS/GCP) 및 저비용 인스턴스 조합으로 월 100만 원 이내 운영 가능하다 | SRS 자체 보완 |
| ASM-05 | 클라우드 크레딧 정책 변동 시 대안 전환 가능. 대안: ① GCP/Azure 무료 크레딧 전환 ② Serverless 전환 ③ Spot/Preemptible 인스턴스 ④ 경량 모델(LightGBM) 대체 | SRS 자체 보완 |
...
| **AOS (Adjusted Opportunity Score)** | 사용자 체감 Pain 강도를 반영한 기회 점수 | PRD §2-3 |
| **DOS (Discovered Opportunity Score)** | 시장 파급력을 반영한 기회 점수 | PRD §2-3 |
...
| **Leading Metrics (선행 지표)** | 최종 재무 결과 이전에 시스템 성공 여부를 예측할 수 있는 사용자 행동 지표 | PRD §1-3 |
| **1-Pass 통과** | 실무자의 기안이 수정 없이 경영진에게 즉시 승인되는 상태 | PRD §1-3 |
...
| ID | 문서 / 출처 | 설명 |
|---|---|---|
| **REF-01** | PRD v1.0 (2026-04-16) | 본 SRS의 원천 문서 (Product Requirements Document) |
| **REF-02** | ISO/IEC/IEEE 29148:2018 | Systems and software engineering — Requirements engineering |
| **REF-03** | ADR-01: 모델 자체 내재화 결정 | SHAP 커스텀 변수 추출을 위해 AWS Forecast 대신 자체 N-BEATS 채택 |
| **REF-04** | ADR-02: Puppeteer 기반 PDF 출력 | 웹 UI 차트를 그대로 렌더 캡처하여 엑셀 세대 체감 만족도 보장 |
...
|---|---|---|---|---|---|
| STK-01 | 이커머스 MD | 김아름 | 외부 변수 반영 발주량 결정, 경영진 결재 품의서 작성 | 결품/과발주 방어, 엑셀 야근 탈출, 결재 1-pass 달성, 독박 책임 면책 | PRD §2-1 Core 1 |
| STK-02 | 3PL 물류센터장 | 정동환 | 화주사 출고량 기반 일용직 인원 확정, 출고 관리 | 오버 스케줄링 인건비 절감, 출고 지연 방지, 화주 기습 프로모션 대응 | PRD §2-1 Core 2 |
...
|---|---|---|---|---|---|
| EXT-01 | 기상청 단기예보 API | Inbound | REST/HTTPS | 지역별 기온·강수·풍속 데이터 수집. 일 1회 메인 배치 덤프 | PRD §6-2, REF-05 |
| EXT-02 | 기상청 중기예보 API | Inbound | REST/HTTPS | 3~10일 예보 데이터 수집. 업데이트 주기 6시간 | PRD §6-2 |
| EXT-03 | 카페24 주문/재고 API | Inbound | REST/HTTPS (OAuth 2.0) | 쇼핑몰 주문·재고 데이터 수집. Rate Limit 분당 제한 | PRD §6-2, REF-06 |
| EXT-04 | 스마트스토어 API | Inbound | REST/HTTPS (OAuth 2.0) | 쇼핑몰 주문·상품 데이터 수집 | PRD §6-2, REF-07 |
| EXT-05 | 네이버 DataLab 트렌드 API | Inbound | REST/HTTPS | 키워드 검색량 지수 수집 | PRD §6-2, REF-08 |
| EXT-06 | 카카오 알림톡 API | Outbound | REST/HTTPS | 센터장·인력소 자동 알림 발송. 건당 과금, 템플릿 사전 승인 | PRD §6-2, REF-09 |
| EXT-07 | 클라우드 인프라 (AWS/GCP) | Infrastructure | — | 무료 티어 + 저비용 인스턴스 조합 운영 | PRD §6-3 |
...
|---|---|---|---|---|---|---|---|
| API-INT-01 | 예측 엔진 API | Internal | POST | `/api/v1/forecast` | 수집 데이터 + 파라미터 | 예측값 + SHAP 해설 + confidence_level | PRD §6-2 |
...
sequenceDiagram
    ...
    participant ML as 예측 엔진 (N-BEATS)
...
|---|---|---|---|---|
| **REQ-FUNC-001** | 시스템은 기상청 단기예보 API로부터 지역별 기온·강수·풍속 데이터를 일 1회 이상 자동 수집하여 Data Lake에 저장해야 한다. | Story 1, F1 | Must | **Given** 기상청 API 연결이 정상 상태일 때 **When** Airflow DAG이 실행되면 **Then** WEATHER_DATA 엔터티에 저장된다. 실패 시 자동 재시도 3회 후 Slack 알림. |
...
| **REQ-FUNC-004** | 시스템은 N-BEATS 모델 기반으로 SKU별 수요 예측값과 **신뢰도(confidence_level)**를 산출해야 한다. | Story 1, F1 | Must | **Given** 기상·트렌드·매출 데이터 수집 완료일 때 **When** 예측 엔진 API 호출되면 **Then** predicted_qty + confidence_level이 FORECAST 엔터티에 저장된다. |
...
| **REQ-FUNC-007** | 사용자가 '리포트 생성' 버튼 클릭 시, 예측 결과 + XAI 해설이 포함된 발주 권장 PDF를 자동 생성해야 한다. | Story 1 | Must | **Given** 데이터 수집 완료 상태일 때 **When** 사용자가 '리포트 생성' 클릭하면 **Then** PDF 생성 완료. 소요시간 **≤ 10분** (p95). |
| **REQ-FUNC-008** | PDF 레이아웃은 파일럿 고객의 **기존 엑셀 품의서 양식**(엑셀 표 + 요약 텍스트)과 호환 포맷이어야 한다. | Story 1 AC 1.2 | Must | **Given** PDF 생성 상태일 때 **When** 경영진이 검토하면 **Then** 기존 결재 양식 대비 레이아웃 거부감 최소화. |
...
| **REQ-FUNC-026** | 시스템은 멀티테넌트 환경에서 테넌트별 데이터를 논리적 격리(스키마)해야 한다. | PRD §5-2 | Must | **Given** 복수 테넌트 사용 시 **When** 테넌트 A가 조회하면 **Then** A 데이터만 반환, 타 테넌트 접근 불가. |
| **REQ-FUNC-027** | 시스템은 Airflow 기반 ETL로 외부 API 수집 → 변환 → 적재를 자동화해야 한다. | PRD §6-3 | Must | **Given** DAG 정의 상태일 때 **When** 스케줄 도달하면 **Then** 수집→정규화→적재 수행. 실패 시 재시도 3회 + Slack 알림. |
...
|---|---|---|---|---|
| **REQ-NF-001** | XAI 리포트 생성 end-to-end 소요시간 (p95) | ≤ 10분 | APM 대시보드 | PRD §5-1 |
| **REQ-NF-002** | API 연동 데이터 수집 지연 (p95) | ≤ 5분 | ETL 모니터링 | SRS 자체 보완 |
| **REQ-NF-003** | 대시보드 페이지 로드 (p95) | ≤ 2초 | RUM | PRD §5-1 |
| **REQ-NF-004** | 예측 모델 추론 (p95) | ≤ 30초 | 모델 서빙 로그 | SRS 자체 보완 |
| **REQ-NF-005** | 카카오 알림톡 발송 (p95) | ≤ 10초 | 알림 발송 로그 | SRS 자체 보완 |
| **REQ-NF-006** | SMS 폴백 전환 소요시간 | ≤ 1분 | 알림 발송 로그 | PRD §3 Story 2 AC 2.4 |

#### 4.2.2 가용성 및 신뢰성 (Availability & Reliability)

| ID | 요구사항 | 임계치 | PRD 참조 |
|---|---|---|---|
| **REQ-NF-007** | 월 가용성 (SLA) | ≥ 99.5% | PRD §5-1 |
| **REQ-NF-008** | 예측 모델 추론 오류율 | ≤ 0.5% | SRS 자체 보완 |
| **REQ-NF-009** | 기상청 API 수집 실패 시 폴백 | 최근 24시간 캐시 자동 전환 + 엠블럼 표기 | PRD §3 AC 1.4 |
| **REQ-NF-010** | ETL 파이프라인 실패 시 | 자동 재시도 3회 + Slack 알림 | PRD §5-1 |
| **REQ-NF-011** | 알림톡 발송 실패 시 SMS 폴백 성공률 | ≥ 99% | PRD §3 AC 2.4 |

#### 4.2.3 보안 (Security)

| ID | 요구사항 | 상세 | PRD 참조 |
|---|---|---|---|
| **REQ-NF-012** | 데이터 전송 암호화 | TLS 1.3 적용 | SRS 자체 보완 |
| **REQ-NF-013** | 데이터 저장 암호화 | AES-256 적용 | SRS 자체 보완 |
| **REQ-NF-014** | 인증 및 접근 제어 | OAuth 2.0 + JWT, RBAC | PRD §5-2 |
| **REQ-NF-015** | 화주사 데이터 격리 | 멀티테넌트 논리 격리 (테넌트별 DB 스키마) | PRD §5-2 |

#### 4.2.4 비용 (Cost)

| ID | 요구사항 | 임계치 | PRD 참조 |
|---|---|---|---|
| **REQ-NF-016** | MVP 기간 인프라 월 비용 | 무료~최대 월 100만 원 이내 (클라우드 무료 티어 우선) | SRS 자체 보완 |

#### 4.2.5 모니터링 및 운영 (Monitoring & Operations)

| ID | 요구사항 | 상세 | 알림 기준 | PRD 참조 |
|---|---|---|---|---|
| **REQ-NF-017** | 인프라/APM 모니터링 | CloudWatch + Datadog | CPU > 80%, 메모리 > 85%, 5xx > 1% | SRS 자체 보완 |
| **REQ-NF-018** | ETL 파이프라인 모니터링 | Airflow DAG | DAG 실패 시 즉시 Slack 알림 | SRS 자체 보완 |
| **REQ-NF-019** | 모델 성능 드리프트 | 커스텀 대시보드 | MAPE > 기준선 + 10%p 시 재학습 트리거 | SRS 자체 보완 |

#### 4.2.6 확장성 및 유지보수성

| ID | 요구사항 | 상세 | PRD 참조 |
|---|---|---|---|
| **REQ-NF-020** | 수평 확장 | API 서버 Stateless → Auto Scaling | SRS 자체 보완 |
| **REQ-NF-021** | 모듈 독립성 | 데이터 수집·예측·리포트·알림은 독립 모듈, 개별 배포 가능 | SRS 자체 보완 |
| **REQ-NF-022** | 외부 API 어댑터 패턴 | 카페24·스마트스토어 API 스펙 변경 대비 | PRD §7-1 R2 |

#### 4.2.7 비즈니스 KPI — Leading Metrics (선행 지표)

> PRD v1.0에서 후행 지표(재무 손실 방어율)를 폐기하고, 사용자 행동 기반 **선행 지표(Leading Metrics)**를 북극성으로 전환하였다.

| ID | 요구사항 (선행 지표) | 측정 기준 (Query/Event) | 목표값 | 측정 도구 | PRD 참조 |
|---|---|---|---|---|---|
| **REQ-NF-023** | 권장 리포트 1-Pass 통과율 | XAI 리포트가 경영진 무수정 통과 | ≥ **80%** | 고객 ERP 로그 / Typeform | PRD §1-3 |
| **REQ-NF-024** | 발주량 무수정 채택 비율 | 사용자가 시스템 값을 수정 없이 채택 (export_pdf 시 edited=false) | ≥ **80%** | Amplitude | PRD §1-3 |
| **REQ-NF-025** | 적정 인원 통보 반영률 | 시스템 산출값 vs confirmed_workers 오차 | 오차율 ≤ **5%** | Mixpanel | PRD §1-3 |

#### 4.2.8 페르소나별 Outcome KPI

| ID | 페르소나 | Outcome 항목 | As-Is | To-Be | 측정 주기 | PRD 참조 |
|---|---|---|---|---|---|---|
| **REQ-NF-026** | 김아름 (MD) | 결품/과발주 기회손실액 | 월 2,500만 원 | **250만 이하** | 월간 | PRD §1-4 |
| **REQ-NF-027** | 김아름 (MD) | 기상·엑셀 취합 야근 시간 | 일 4시간 | **10분 컷** | 일간 | PRD §1-4 |
| **REQ-NF-028** | 정동환 (센터장) | 오버 스케줄링 일당 손실 | 일 80만 원 | **16만 이하** | 일간 | PRD §1-4 |
...
| Story 1 | | REQ-NF-023 | TC-N23: 1-Pass 통과율 검증 (≥80%) |
| Story 1 | | REQ-NF-024 | TC-N24: 무수정 채택 비율 검증 (≥80%) |
...
| Story 2 | | REQ-NF-025 | TC-N25: 인원 통보 반영률 검증 (오차 ≤5%) |
...
| 공통 | | REQ-NF-007 | TC-N07: SLA 99.5% 가용성 검증 |
| 공통 | | REQ-NF-012 | TC-N12: TLS 1.3 적용 검증 |
| 공통 | | REQ-NF-015 | TC-N15: 멀티테넌트 격리 검증 |
| 공통 | | REQ-NF-016 | TC-N16: 인프라 월 비용 ≤100만 원 검증 |
...
> **PRD v1.0 변경 반영:** FORECAST 엔터티의 `confidence_interval` → `confidence_level`로 변경 (AC 1.3 신뢰도 경고 트리거). WORKFORCE_PLAN에 `notification_channel` 필드 추가 (알림톡/SMS 채널 구분).
...
|---|---|---|---|---|---|---|---|
| **EXP-01** | 파일럿 PoC (pre-post) | 4개 고객사 MD (김아름 유형) | **대응 표본 T-검정 (Paired t-test)** α=0.05, MDE=소요시간 30% 강하, n≥30 | 의사결정 소요 시간, 1-Pass 통과율 | p < 0.05 환경에서 수동 엑셀 대비 30% 이상 단축 | 21 영업일 | PRD §8-1 EXP-01 |
| **EXP-02** | 파일럿 PoC (pre-post) | 파일럿 물류센터 (정동환 유형) | **대응 표본 T-검정 (Paired t-test)** α=0.05, MDE=인건비 10%p 개선, n=21영업일 | 인건비 절감률, 인원 오차율 | 직감 배치 대비 손실 곡선 축소 입증 | 21 영업일 | PRD §8-1 EXP-02 |
...
*End of SRS-002 v1.0 — PRD v1.0 기반 전면 재작성*
